    ...
    public static function shortcode_to_args($atts = '', $paged = '') {
        ...
        if (!empty($listing_category)) {
            $tax_query[] = array(
                ...
                'field' => 'slug',
                'terms' => self::traverse_comma_string($listing_category)
            );
        }
        if (!empty($listing_transport)) {
            $tax_query[] = array(
                'taxonomy' => 'listing_transport',
                'field' => 'slug',
                'terms' => self::traverse_comma_string($listing_transport)
            );
        }
        if (!empty($room_type)) {
            $tax_query[] = array(
