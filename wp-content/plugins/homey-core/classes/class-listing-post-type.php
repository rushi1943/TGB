...
class Homey_Listing_Post_Type {
    ...
    public static function init() {
		...
		add_action( 'init', array( __CLASS__, 'listing_category' ) );
        add_action( 'init', array( __CLASS__, 'listing_type' ) );
        add_action( 'init', array( __CLASS__, 'room_type' ) );
        add_action( 'init', array( __CLASS__, 'listing_animals' ) );
        add_action( 'init', array( __CLASS__, 'listing_terrains' ) );
        add_action( 'init', array( __CLASS__, 'listing_weapons' ) );
        add_action( 'init', array( __CLASS__, 'listing_transport' ) );
    ...
    public static function definition() {
        $labels = array(
            'name' => esc_html__( 'Tgb Listings','homey-core'),
            'singular_name' => esc_html__( 'Tgb Listing','homey-core' ),
    ...
    public static function listing_category() {
        ...
        $type_labels = array(
                    'name'              => esc_html__('Category','homey-core'),
                    'add_new_item'      => esc_html__('Add New','homey-core'),
                    'new_item_name'     => esc_html__('New Category','homey-core')
        );
        $type_labels = apply_filters( 'listing_category_labels', $type_labels );
        ...
        register_taxonomy('listing_category', 'listing', array(
                ...
                'rest_controller_class' => 'WP_REST_Terms_Controller',
                'rewrite'       => array( 'slug' => homey_get_listing_category_rewrite_slug() )
            )
        );
    }

    public static function listing_transport() {

        $type_labels = array(
                    'name'              => esc_html__('Transport','homey-core'),
                    'add_new_item'      => esc_html__('Add New','homey-core'),
                    'new_item_name'     => esc_html__('New Transport','homey-core')
        );
        $type_labels = apply_filters( 'listing_transport_labels', $type_labels );

        register_taxonomy('listing_transport', 'listing', array(
                'labels' => $type_labels,
                'hierarchical'  => true,
                'query_var'     => true,
                'show_in_rest'          => true,
                'rest_base'             => 'listing_transport',
                'rest_controller_class' => 'WP_REST_Terms_Controller',
                'rewrite'       => array( 'slug' => homey_get_listing_transport_rewrite_slug() )
            )
        );
    }
    ...
    public static function listing_animals() {

        $listing_animal_labels = array(
                    'name'              => esc_html__('Animal Type(s)','homey-core'),
                    'add_new_item'      => esc_html__('Add New','homey-core'),
                    'new_item_name'     => esc_html__('New Animal','homey-core')
        );
        $listing_animal_labels = apply_filters( 'listing_animal_labels', $listing_animal_labels );

        register_taxonomy('listing_animal', 'listing', array(
                'labels' => $listing_animal_labels,
                'hierarchical'  => true,
                'query_var'     => true,
                'show_in_rest'          => true,
                'rest_base'             => 'animals',
                'rest_controller_class' => 'WP_REST_Terms_Controller',
                'rewrite'       => array( 'slug' => homey_get_animal_rewrite_slug() )
            )
        );
    }

    public static function listing_terrains() {

        $listing_terrain_labels = array(
                    'name'              => esc_html__('Terrain','homey-core'),
                    'add_new_item'      => esc_html__('Add New','homey-core'),
                    'new_item_name'     => esc_html__('New terrain','homey-core')
        );
        $listing_terrain_labels = apply_filters( 'listing_terrain_labels', $listing_terrain_labels );

        register_taxonomy('listing_terrain', 'listing', array(
                'labels' => $listing_terrain_labels,
                'hierarchical'  => true,
                'query_var'     => true,
                'show_in_rest'          => true,
                'rest_base'             => 'terrains',
                'rest_controller_class' => 'WP_REST_Terms_Controller',
                'rewrite'       => array( 'slug' => homey_get_terrain_rewrite_slug() )
            )
        );
    }

    public static function listing_weapons() {

        $listing_weapon_labels = array(
                    'name'              => esc_html__('Weapons/Equipment','homey-core'),
                    'add_new_item'      => esc_html__('Add New','homey-core'),
                    'new_item_name'     => esc_html__('New Weapon','homey-core')
        );

        $listing_weapon_labels = apply_filters( 'listing_weapon_labels', $listing_weapon_labels );

        register_taxonomy('listing_weapons', 'listing', array(
                'labels'        => $listing_weapon_labels,
                'hierarchical'  => true,
                'query_var'     => true,
                'show_in_rest'          => true,
                'rest_base'             => 'weapons',
                'rest_controller_class' => 'WP_REST_Terms_Controller',
                'rewrite'       => array( 'slug' => homey_get_weapon_rewrite_slug() )
            )
        );
    }
    ...
    public static function custom_columns_manage( $column ) {
        ...
        switch ($column)
        {
            ...
            case 'address':
                ...
                break;
            case 'address2':
                $address2 = get_post_meta($post->ID, $prefix.'listing_address2',true);
                if(!empty($address2)){
                    echo esc_attr( $address2 );
                }
                else{
                    _e('No Address2 Provided!','homey-core');
                }
                break;                
            case 'type':
                ...
                break;            
			case 'category':
                echo Homey::admin_taxonomy_terms ( $post->ID, 'listing_category', 'listing' );
                break;
            case 'status':
