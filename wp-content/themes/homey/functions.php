...
define( 'HOMEY_THEME_VERSION', '1.4.2.1' );
/**
*	----------------------------------------------------------------------------------
 ...
 *	-----------------------------------------------------------------------------------------
 */
require_once( get_template_directory() . '/insignia-fw/init.php' );/*custom code*/
...
if( !function_exists('homey_delete_property_attachments') ) {
	function homey_delete_property_attachments($postid)
	{
		...
		if ($post_type == 'homey_review') {
			$review_reservation_id = get_post_meta($postid, 'review_reservation_id', true); 
