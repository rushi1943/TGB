...
if ( isset ($_POST['reservation_pay']) && $_POST['reservation_pay'] == 1  ) {
    try {
        ...
        $is_extra_options  = intval($_POST['extra_options']);
        ...
        if($is_instance_booking == 1) {
            $stripe_des = esc_html__( 'Listing ID','homey').' '.$_POST['listing_id'];
        }
        ...
        if(homey_is_renter()) {
            ...
            $return_link = add_query_arg( 'reservation_detail', $reservation_id, $reservation_page_link_host );
        }
    ...
    try {
        ...
        $charge = \Stripe\Charge::create(array(
            ...
            'customer' => $customer->id,
            "currency" => $submission_currency,
            "description" => esc_html__( 'Listing ID','homey').' '.$listing_id,
        ));
