...
if( !function_exists('homey_listing_filter_callback') ) {
    function homey_listing_filter_callback( $lsiting_qry ) {
        ...
        $animals = get_post_meta( $page_id, $prefix.'animals', false );
        if ( ! empty( $animals ) && is_array( $animals ) ) {
            $tax_query[] = array(
                'taxonomy' => 'listing_animal',
                'field' => 'slug',
                'terms' => $animals
            );
        }

        $terrains = get_post_meta( $page_id, $prefix.'terrains', false );
        if ( ! empty( $terrains ) && is_array( $terrains ) ) {
            $tax_query[] = array(
                'taxonomy' => 'listing_terrain',
                'field' => 'slug',
                'terms' => $terrains
            );
        }
...
if( !function_exists('listing_submission_filter')) {
    function listing_submission_filter($new_listing) {
        ...
        if(isset($_POST['post_author_id']) && !empty($_POST['post_author_id']) ) {
            $new_listing['post_author'] = $_POST['post_author_id'];
        } else {
        ...
        if( $listing_id > 0 ) {
            ...
            // Address2
            if( isset( $_POST['listing_address2'] ) ) {
                update_post_meta( $listing_id, $prefix.'listing_address2', sanitize_text_field( $_POST['listing_address2'] ) );
            }

            // Phone
            if( isset( $_POST['listing_phone'] ) ) {
                update_post_meta( $listing_id, $prefix.'listing_phone', sanitize_text_field( $_POST['listing_phone'] ) );
            }

            // URL
            if( isset( $_POST['listing_url'] ) ) {
                update_post_meta( $listing_id, $prefix.'listing_url', sanitize_text_field( $_POST['listing_url'] ) );
            }

            // More Guid
            if( isset( $_POST['listing_more_guid'] ) ) {
                update_post_meta( $listing_id, $prefix.'listing_more_guid', sanitize_text_field( $_POST['listing_more_guid'] ) );
            }

            // Guid
            if( isset( $_POST['listing_guid'] ) ) {
                $listing_guid = maybe_serialize($_POST['listing_guid']);
                update_post_meta( $listing_id, $prefix.'listing_guid', sanitize_text_field($listing_guid) );
            }

            // Trip Date
            if( isset( $_POST['trip_date'] ) ) {
                update_post_meta( $listing_id, $prefix.'trip_date', sanitize_text_field( $_POST['trip_date'] ) );
            }

            // Trip list for all date
            if( isset( $_POST['listing_price_all_date'] ) ) {
                update_post_meta( $listing_id, $prefix.'listing_price_all_date', sanitize_text_field( $_POST['listing_price_all_date'] ) );
            }

            // Trip Run Deal
            if( isset( $_POST['listing_run_deal'] ) ) {
                update_post_meta( $listing_id, $prefix.'listing_run_deal', sanitize_text_field( $_POST['listing_run_deal'] ) );
            }

            // Price
            if( isset( $_POST['listing_price'] ) ) {
                update_post_meta( $listing_id, $prefix.'listing_price', sanitize_text_field( $_POST['listing_price'] ) );
            }

            // Unlimited Participants
            if( isset( $_POST['listing_unlimited_participants'] ) ) {
                update_post_meta( $listing_id, $prefix.'listing_unlimited_participants', sanitize_text_field( $_POST['listing_unlimited_participants'] ) );
            }

            //Participants
            if( isset( $_POST['listing_participants'] ) ) {
                update_post_meta( $listing_id, $prefix.'listing_participants', sanitize_text_field( $_POST['listing_participants'] ) );
            }

            //All Age
            if( isset( $_POST['listing_age_all'] ) ) {
                update_post_meta( $listing_id, $prefix.'listing_age_all', sanitize_text_field( $_POST['listing_age_all'] ) );
            }

            //All Age
            if( isset( $_POST['listing_age_family_friendly'] ) ) {
                update_post_meta( $listing_id, $prefix.'listing_age_family_friendly', sanitize_text_field( $_POST['listing_age_family_friendly'] ) );
            }

            //Age
            if( isset( $_POST['listing_age'] ) ) {
                update_post_meta( $listing_id, $prefix.'listing_age', sanitize_text_field( $_POST['listing_age'] ) );
            }

            //Loadging
            if( isset( $_POST['listing_loadge'] ) ) {
                update_post_meta( $listing_id, $prefix.'listing_loadge', sanitize_text_field( $_POST['listing_loadge'] ) );
            }

            //Loadging Description
            if( isset( $_POST['listing_loadge_description'] ) ) {
                update_post_meta( $listing_id, $prefix.'listing_loadge_description', sanitize_text_field( $_POST['listing_loadge_description'] ) );
            }
            ...
            if( isset( $_POST['cancellation_policy'] ) ) {
                update_post_meta( $listing_id, $prefix.'cancellation_policy', ( $_POST['cancellation_policy'] ) );
            }
            ...
            if( isset( $_POST['listing_category'] ) ) {
                ...
                foreach( $_POST['listing_category'] as $category_id ) {
                    $categories_array[] = intval( $category_id );
                }
                wp_set_object_terms( $listing_id,$categories_array, 'listing_category' );
            }

            // animals
            if( isset( $_POST['listing_animal'] ) ) {
                $animals_array = array();
                foreach( $_POST['listing_animal'] as $animal_id ) {
                    $animals_array[] = intval( $animal_id );
                }
                wp_set_object_terms( $listing_id, $animals_array, 'listing_animal' );
            }

            // terrains
            if( isset( $_POST['listing_terrain'] ) ) {
                $terrains_array = array();
                foreach( $_POST['listing_terrain'] as $terrain_id ) {
                    $terrains_array[] = intval( $terrain_id );
                }
                wp_set_object_terms( $listing_id, $terrains_array, 'listing_terrain' );
            }
    ...
    function homey_create_print () {
        ...
        <div class="print-main-wrap">
            <div class="print-wrap">
                ...
                <?php
                $animals   = wp_get_post_terms( $listing_id, 'listing_animal', array("fields" => "all"));
                $terrains  = wp_get_post_terms( $listing_id, 'listing_terrain', array("fields" => "all"));
                ?>
                        ...
                        <div class="block-section">
                            <div class="block-body">
                                ...
                                <div class="block-right">
                                    ...
                                    <p><strong><?php echo esc_attr($homey_local['animals']); ?></strong></p>
                                    <ul class="detail-list detail-list-2-cols">
                                    ...
                                    <p><strong><?php echo homey_option('sn_terrains'); ?></strong></p>
                                    <ul class="detail-list detail-list-2-cols">
